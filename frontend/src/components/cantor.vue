  ...
  <el-container class="page-cantor" v-loading="loading" :style="areaStyle">
    ...
    <el-main class="main-area">
      ...
      >
        <el-form ref="form" :model="datas.config" :rules="rules" label-width="60px">
          <el-form-item label="仓库" prop="repo">
            <el-input v-model="datas.config.repo" placeholder="cantor"></el-input>
          </el-form-item>
      ...
      >
        ...
        <div>
          <el-link
            @click="onOpenUrl('https://github.com/evercyan/cantor')"
            type="primary"
          >
        ...
        <el-steps
          ...
          class="step-area"
        >
          <el-step title="配置" description="点击左侧 `设置` 按钮设置 GitHub 相关配置"></el-step>
          <el-step title="上传" description="点击左侧 `上传` 按钮可选择图片文件上传到 GitHub 仓库"></el-step>
          <el-step title="使用" description="点击列表 `复制` 按钮复制文件链接"></el-step>
        </el-steps>
      </el-drawer>
    </el-main>
  </el-container>
  ...
  methods: {
    ...
    goGetConfig: function () {
      this.app.GetConfig().then((resp) => {
        ...
        this.datas.config = resp.data.config;
    ...
    goUploadFile() {
      this.loading = true;
      this.app.UploadFile().then((resp) => {
        this.log('Go', 'UploadFile', resp);
        this.loading = false;
        if (resp.code !== 0) {
          this.$message.error(resp.msg);
          return;
